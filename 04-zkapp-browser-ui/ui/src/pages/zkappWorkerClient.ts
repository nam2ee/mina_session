...
export default class ZkappWorkerClient {
  ...
  async getAnswer(): Promise<Field> {
    const result = await this._call('getAnswer', {});
    return Field.fromJSON(JSON.parse(result as string));
  }

  createSetAnswerTransaction(answer: number ) {
    return this._call('createSetAnswerTransaction', { answer });
  }

  createGuessTransaction(guess: number) {
    return this._call('createGuessTransaction', { guess });
  }
